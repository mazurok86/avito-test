...
- **Таргет-чат** — `TARGET_USER_NAME` ищется как case-insensitive подстрока в
  ...
  корне списка (debounce 2 секунды, чтобы схлопывать всплески мутаций — typing
  indicators, read receipts; жёсткий потолок 10 секунд, после которого
  накопленное окно force-flush'ится, иначе при непрерывных мутациях refresh
  не сработал бы никогда) плюс safety-таймер раз в 60 секунд (на случай
  навигаций, отвязывающих observer).
- **Буфер сообщений до появления чата в DOM** — если WS-фрейм пришёл раньше,
  чем `refreshTargetChats` подхватил новый канал, сообщение откладывается в
  per-channel очередь с TTL = 2× периода health-монитора (120 с). Как только
  очередной скан списка чата добавит канал в `targetChats`, накопленные
  сообщения flush'атся в обычный конвейер.
...
`DEBUG_SCREENSHOTS=true` включает дамп полностраничных PNG + HTML-снэпшотов
на каждой ключевой точке auth-флоу (`./debug-screenshots/<ts>-<label>.{png,html}`).
По умолчанию выключено — на хосте без X-сервера или при долгой работе сервиса
скриншоты не нужны и быстро разрастаются.
